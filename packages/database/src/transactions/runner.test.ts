...
import { mintTrustedWorkerAccess } from "../security";
...
describe("persistence transaction runner (live local Postgres)", () => {
  it("does not leak tenant context to the next transaction on a reused connection", async (ctx) => {
    ...
    try {
      ...
      const first = await runner.run({
        access: mintTrustedWorkerAccess(scopeFor("tenant-a")),
        work: async (tx) => {
    ...
    try {
      ...
      const results = await Promise.all(
        tenantIds.map((tenant) =>
          runner.run({
            access: mintTrustedWorkerAccess(scopeFor(tenant)),
            work: async (tx) => {
  ...
  it("rolls back a failed transaction and releases its connection", async (ctx) => {
    ...
    try {
      ...
      const failed = await runner.run({
        access: mintTrustedWorkerAccess(scopeFor("tenant-rollback")),
        work: async (tx) => {
          const db = resolveTransactionExecutor(tx);
          await db.execute(
      ...
      const second = await runner.run({
        access: mintTrustedWorkerAccess(scopeFor("tenant-after-rollback")),
  ...
  it("stops exposing the executor once a transaction has settled", async (ctx) => {
    ...
    try {
      ...
      await runner.run({
        access: mintTrustedWorkerAccess(scopeFor("tenant-settled")),
        work: (tx) => {
          capturedTx = tx;
