...
1. Use the private application repository `maximebrmd/relanmo`, created at the user's request. The full technical pack is already at `planning/prospecting-system/`, with root AGENTS.md and an initial commit. Clone/register that repository in Firstmate; preserve its Git history, planning pack and brand assets when P001 scaffolds next-forge.
