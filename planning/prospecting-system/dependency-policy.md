...
4. Check the chosen library's current official documentation for compatibility and API syntax. A next-forge migration example may target an older major release. Preserve the user's choices of Better Auth, Drizzle, Ultracite with Oxlint/Oxfmt, and Bun when adapting examples.
