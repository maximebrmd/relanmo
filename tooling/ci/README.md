...
The test job sets `DATABASE_TEST_ADMIN_URL` to its disposable Postgres service. The database package's `test` script invokes `run-database-tests.mjs`, which runs the database and auth schema Vitest files once and rejects skipped or undiscovered tests in CI. Each database test process creates a unique database name and drops it after the test, so a missing service is an error in CI rather than a silent skip.
